...
$date = date("Y-m-d");
$title=mysqli_real_escape_string($conn1,$_POST['title']);
 ...
 $organization = mysqli_real_escape_string($conn1,$_POST['organization']);
 $country = mysqli_real_escape_string($conn1,$_POST['country']);
 $session = mysqli_real_escape_string($conn1,$_POST['session']);
 $tracks = mysqli_real_escape_string($conn1,$_POST['tracks']);
  $address = mysqli_real_escape_string($conn1,$_POST['address']);
      ...
      if(empty($errors)==true){
         if(move_uploaded_file($file_tmp,"../uploads/abstracts-files/".$file_name))
         {
mysqli_query($conn1,"INSERT INTO `abstracts_tblstr_d`(`ip`, `user`, `title`, `name`, `email`, `phone`, `organization`, `country`, `session`, `tracks`, `file_name`, `address`, `created_date`) VALUES ('$ip', '".$r3['username']."', '$title','$name','$email','$phone','$organization','$country','$session','$tracks','$file_name','$address','$date')");
echo '<script>window.location="submit-abstract-confirmation"</script>';
      }}
	  echo 'Error';
...
@$fileatt_name = @$file_name; // Filename that will be used for the file as the attachment
          ...
          // @$recipientbody ="Title : ".$_POST['title']." ".$_POST['name']."\nCountry : ".$_POST['country']."\nE-mail : ".$_POST['email']."\nPhone Number : ".$_POST['phone']."\nInterested in : ".$_POST['session']."\nOrganization : ".$_POST['organization']."\nTrack Name : ".$_POST['tracks'];
		   ...
		   @$recipientbody ="Title : ".$_POST['title']." ".$_POST['name']."\nCountry : ".$_POST['country']."\nE-mail : ".$_POST['email']."\nPhone Number : ".$_POST['phone']."\nOrganization : ".$_POST['organization']."\Interested In : ".$_POST['session']."\nTrack Name : ".$_POST['tracks'];
...
$recipientbody .= "--{$mime_boundary}\n" .
              ...
              "--{$mime_boundary}--\n";
			  @$mailheader2 = "From:  AGMEETS < $recipient >\n"; 
            @$mailheader2 .= "Reply-To:  AGMEETS  < $recipient >\n"; 
                                       ...
                                       <div class="form-group"> 
                                          <label for="lname">Select Track<span style="color: red;">*</span></label>									   
                                                    <select name="tracks" class="form-control">
                                                         <?php
$query=mysqli_query($conn1,"SELECT * FROM `topics_tblstr_d` WHERE user='$id' AND `status`='1' order by track_id ASC");
...
$query=mysqli_query($conn1,"SELECT * FROM `topics_tblstr_d` WHERE user='$id' AND `status`='1' order by track_id ASC");
while($data=mysqli_fetch_array($query)){?>
                                        <li> <?php echo $data['track_name'];?></li>
